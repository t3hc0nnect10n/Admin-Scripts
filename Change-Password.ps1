...
if (Test-Path -Path $ImagePath -ErrorAction SilentlyContinue) {
	...
	$Form.Controls.AddRange(@($Label_, $Labelp, $Label0, $Label1, $Label2, $Label3, $TextBox1, $TextBox2, $TextBox3, $StatusLabel0, $StatusLabel1, $StatusLabel2, $Button, $PictureBox))
}
else {

	# Добавляем в форму установленные параметры без изображения.
	$Form.Controls.AddRange(@($Label_, $Labelp, $Label0, $Label1, $Label2, $Label3, $TextBox1, $TextBox2, $TextBox3, $StatusLabel0, $StatusLabel1, $StatusLabel2, $Button))
}
...
$Button.add_click({
	...
	# Отображение в форме сгенерированного пароля в качестве примера.
